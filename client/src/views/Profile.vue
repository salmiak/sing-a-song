<template lang="html">
  <div>
    ...
    <template v-if="profile">
      <!-- Profile Cover -->
      <v-parallax v-if="profile.coverURL" height="320" :src="profile.coverURL">
        <v-toolbar flat color="transparent" dark class="flex-grow-1">
          <router-link to="/">
      ...
      <v-toolbar v-if="!profile.coverURL" flat color="transparent">
        <router-link to="/">
          <v-avatar size="32" class="mr-4">
            <svg
              class="appbar-logo"
    ...
    <v-container class="pb-16">
      ...
      <template v-if="profile">
        ...
        <v-row justify="center">
          <v-col class="text-center col-12 col-sm-8">
            <h1 class="text-h3 mb-4">{{ profileName }}</h1>

            <vue-markdown v-if="profile.description">
              {{ profile.description }}
            </vue-markdown>
            ...
            <vue-markdown v-if="profile.contactDetails">
  ...
  export default {
    ...
    computed: {
      ...
      profileName() {
        if (!this.$store.state.profiles.all.items) return undefined;
        return (
          this.profile.stageName ||
          this.profile.user.firstName + " " + this.profile.user.lastName
        );
      },
      profileSlug() {
      ...
      showEdit() {
        if (!this.$store.state.authentication.user) return false;
    ...
    methods: {
      ...
      removeImg(target) {
        ...
        this.$store.dispatch("profiles/update", payload).then(response => {
