<template>
  <form-view title="Logga in" :alert="this.alert" @submitted="handleSubmit">
    <v-text-field
      type="email"
      v-model="email"
      label="Email"
    ...
    <v-text-field
      ...
      outlined
    >
      <v-icon slot="append" @click="showPassword = !showPassword" tabindex="-1">
        {{ showPassword ? "mdi-eye-off" : "mdi-eye" }}
      </v-icon>
    </v-text-field>
    <v-btn color="primary" :disabled="loggingIn" @click="handleSubmit"
      >Logga in</v-btn
    >
    ...
    <p class="mt-8">
      <router-link to="/forgot-password">Glömt lösenord</router-link> - Om du
      inte har något konto så
      <router-link to="/register">skapa ett här</router-link>.
    </p>
  </form-view>
</template>
